  ...
  try {
    const [statsRows, porModalidade, porEmpresa, atendimentos] = await Promise.all([
      sql`
        SELECT
          ...
          (SELECT COUNT(*) FROM atendimentos WHERE status = 'Faltou') as faltou,
          (SELECT COUNT(*) FROM atendimentos WHERE status = 'Reagendado') as reagendados,
          (SELECT COUNT(*) FROM atendimentos WHERE status = 'Confirmado') as confirmados,
    ...
    return jsonOk({
      stats: {
        ...
        faltou: parseInt(stats[0]?.faltou ?? "0"),
        reagendados: parseInt(stats[0]?.reagendados ?? "0"),
        confirmados: parseInt(stats[0]?.confirmados ?? "0"),
